...
def get_data_input_tensor(batch_size=16):
    ...
    with tf.device('/gpu:0'):
        ...
        tf_data = tf_data.map(lambda img, kp :
                                tf.py_func(transform_data, [img, kp], 4*[tf.float32], stateful=False),
                                num_parallel_calls=64)
        #print(tf_data)
        tf_data = tf_data.batch(batch_size)
        #print(tf_data)
        tf_data = tf_data.prefetch(10)
        #print("Prefetch",tf_data)
